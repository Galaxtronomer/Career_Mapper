...
<head>
<meta charset="utf-8" />
<title>Career Map Editor with Auto-Save</title>
...
function saveData() {
    const data = {
        drops: drops.map(m => m.getLatLng()),
        texts: texts.map(t => ({
            latlng: t.getLatLng(),
            html: t._icon.innerHTML,
            fontSize: t._icon.style.fontSize,
            color: t._icon.style.color,
            rotation: t.options.rotationAngle
        })),
        curves: curves.map(c => ({
            start: c.start,
            end: c.end,
    ...
    localStorage.setItem('careerMapData', JSON.stringify(data));
}

function loadData() {
    const data = JSON.parse(localStorage.getItem('careerMapData') || '{}');
    
    if(data.drops) data.drops.forEach(latlng => {
        const marker = L.marker(latlng,{draggable:true}).addTo(map);
        marker.on('dragend', saveData);
        drops.push(marker);
    });
    
    if(data.texts) data.texts.forEach(tData => {
        const t = L.marker(tData.latlng,{ 
            icon:L.divIcon({className:'custom-text',html:tData.html,iconAnchor:[0,0],
            style: `font-size:${tData.fontSize}; color:${tData.color}`}),
            draggable:true, rotationAngle: tData.rotation || 0, rotationOrigin: 'center'
        }).addTo(map);
        t.setRotation = function(angle){ t.options.rotationAngle = angle; t._icon.style.transform = 'rotate('+angle+'deg)'; };
        t.on('dragend', saveData);
        texts.push(t);
    });
    
    if(data.curves) data.curves.forEach(cData => {
        const curve = { ...cData, poly:null, arrow:null, startMarker:null, endMarker:null, controlMarker:null };
        drawCurve(curve);
        curves.push(curve);
    });
...
document.getElementById('removeDropBtn').onclick = function(){
    ...
    if(idx!=""){ map.removeLayer(drops[idx]); drops.splice(idx,1); updateDropList(); saveData(); }
};
...
document.getElementById('removeTextBtn').onclick = function(){
    ...
    if(idx!=""){ map.removeLayer(texts[idx]); texts.splice(idx,1); updateTextList(); saveData(); }
};
document.getElementById('rotateTextBtn').onclick = function(){
    var idx = document.getElementById('textList').value;
    if(idx==="") return;
    var angle = prompt("Enter rotation in degrees:",0);
    if(angle!==null){ texts[idx].setRotation(parseFloat(angle)); saveData(); }
};
...
filename = "career_map_autosave.html"
